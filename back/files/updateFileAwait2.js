...
module.exports = {
    updateFileAwait2: async function (req, res, body, prices, Files, Actions) {
        const thisFile2 = await Files.findOne({where: {id: body.id}});
        // console.log(thisFile2);
        if (thisFile2 !== null) {
            thisFile = {
                id: thisFile2.id,
                name: thisFile2.name,
            ...
            if(parameter1){
                if (parameter1 === "paper") {
                    if (body.value === "Самоклеючі") {
                        Object.defineProperty(thisFile, parameter1, {
                            value: body.value,
                            writable: true
                        })
                        thisFile.destiny = 'Біла самоклеюча плівка'
                        // thisFile.format = 'A3'
                        // thisFile.cuttingSamokleika = 'з фігурною порізкою (порізка продукції на аркуші форматом А3)'
                        thisFile.lamination = null
                        thisFile.roundCorner = null
                        ...
                        // thisFile.format = 'A4'
                        thisFile.destiny = null
                        thisFile.cuttingSamokleika = null
            ...
            if (thisFile.calc === "digital") {
                if (thisFile.format === "A4" || thisFile.format === "A3") {
                    ...
                    thisFile.realCount = Math.ceil(thisFile.count * thisFile.countInFile / getHowInASheet())
                ...
                let cuttingSamokleikaPrice = getPriceFromCount(thisFile.cuttingSamokleika, "Порізка самоклейки", "", prices) * thisFile.realCount
            ...
            const updatedFile = await Files.update(thisFile, {where: {id: body.id}});
            // let ress = {
            //     url: thisFile.path,
            //     img: thisFile.img,
            //     red: thisFile.red
            ...
            res.send(sendData);
            log.addStatistics(req.userId, req.sessionId, `update file: `+body, "success", body.id, thisFile.price, Actions)
        } else {
            res.send({
                status: "error",
